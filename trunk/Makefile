...
ACLOCAL = /home/debayan/test/config/missing aclocal-1.4
AUTOCONF = /home/debayan/test/config/missing autoconf
AUTOMAKE = /home/debayan/test/config/missing automake-1.4
AUTOHEADER = /home/debayan/test/config/missing autoheader
...
LTLIB = @LTLIB@
MAINT = #
MAKEINFO = /home/debayan/test/config/missing makeinfo
PACKAGE = 
PACKAGE_DATE = 04/22
...
Makefile.in NEWS acinclude.m4 aclocal.m4 config/config.h.in \
config/stamp-h.in configure configure.ac
...
config_auto.h: stamp-h
	@if test ! -f $@; then \
		rm -f stamp-h; \
		$(MAKE) stamp-h; \
	else :; fi
stamp-h: $(srcdir)/config/config.h.in $(top_builddir)/config.status
	cd $(top_builddir) \
	  && CONFIG_FILES= CONFIG_HEADERS=config_auto.h:config/config.h.in \
	     $(SHELL) ./config.status
	@echo timestamp > stamp-h 2> /dev/null
$(srcdir)/config/config.h.in: #$(srcdir)/stamp-h.in
	@if test ! -f $@; then \
...
all-recursive-am: config_auto.h
...
#EXTRA_DIST = doc/html doc/tesseract_2.03.pdf doc/tesseract_2.03.ps.gz
