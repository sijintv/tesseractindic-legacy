...
A_CHOICE *permute_all(CHOICES_LIST char_choices,
  ...
  if (permute_only_top)
    return result_1;
  //cprintf("\nany_alpha->%d -- (array_count (char_choices) <= MAX_WERD_LENGTH)-->%d)\n",any_alpha ,(array_count (char_choices) <= MAX_WERD_LENGTH));
  any_alpha=1;
