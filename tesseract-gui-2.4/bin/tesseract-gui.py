...
class Whc:
	def __init__(self):
		...
		self.mainwindow.set_title("Tesseract GUI 2.3")
	...
	###---Left of Window-------------------------------------
		...
		self.imgIcon.set_from_file("/usr/share/tesseract-gui/tesseract-gui-logo.png")
		self.headHBox.pack_start(self.imgIcon,False,False,1)
		self.imgIcon.show()
		
		self.lblHead=gtk.Label()
		self.lblHead.set_markup("<b>Tesseract-GUI</b>")
...
# Pack-ends
		...
		self.btnConcat.set_current_folder(os.getenv('HOME'))
		...
		self.btnTrain = gtk.Button("Train")
                self.vboxOcr.pack_end(self.btnTrain, False, False, 1)
                self.btnTrain.connect("clicked", self.f_train)
                self.btnTrain.show()
		...
		self.btnPrev = gtk.Button("",gtk.STOCK_ZOOM_OUT,False)
                self.btnPrevMinus.connect("clicked", self.f_show_img,None,None,0.9,False)
                self.hboxPrevOp.pack_start(self.btnPrevMinus, False, False, 1)
                self.btnPrevMinus.show()
...
### Functions-------------------------------------------------------------------
	...
	def f_load_files(self, widget, Data):
		if Data == -5 :
			...
			#self.imageName = self.dlgFiles.get_current_folder()+self.Files[0]
	...
	def f_show_img(self,widget,Data,Data2,Zoom,WritePrev):
		...
		if self.Selected != None:
			...
			self.imageName = self.dlgFiles.get_current_folder()+"/"+(self.Images[self.Selected])[2]+".tif" #for tesseractTrainer.py
		...
		if len(ImagesToProcess) > 0 and self.dialogInfo:
			...
			for nn in range(0,nnfin):
				...
				Comprueba = runBash("ls " + self.FolderOut \
						+ NameOut + ".txt 2> /dev/null")
				if Comprueba :
					self.dialogInfo.change("error",self.LCabort1+"<b>"\
						+ self.FolderOut + NameOut+"</b>","show")
...
#					runBash("rm " + Processtif  + " ;")
	...
	def f_create_lang(self):
		Languages = { self.LCspsanish:"spa",\
					self.LCportuguese:"por",\
					self.LCitalian:"ita",\
					self.LCdutch:"nld",\
					self.LCgerman:"deu",\
					self.LCfrench:"fra",\
	...
	def f_lang_choice(self , widget): 
		...
		if active < 0:
			return None
		self.lang = self.ListLanguages[(model[active][0])]
		self.ConfVars["Language"]= self.lang + "\n"
		self.f_write_conf()
	# f_lang_coice END
	...
	class Dialogo:
		def create(self,Type,Data,Show):
			...
			self.processdialog.set_size_request(340,150)
			...
			if Type == "info" :
				...
				self.imgIcon.set_from_file("/usr/share/tesseract-gui/tesseract-gui-error.png")
